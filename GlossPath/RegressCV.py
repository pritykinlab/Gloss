...
class RegressCV():
    ...
    def __init__(self, adata, resolutions, pathway_string,
                 alphas = [0.004, 0.005, 0.006, 0.007], l1s = [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5], donors_profiled=False):
        self.pathway_string = pathway_string
        self.prepped_data = PrepData(adata, self.pathway_string, donors_profiled=donors_profiled)
        self.alphas = alphas 
        self.l1s = l1s
    ...
    def _cv_loop(self):
        ...
        for res in self.resolutions:
            ...
            for ctype in self.resolutions[res]:
                ...
                myreg = Regressor(gene_list = self.adata.var.index, pathway_string=self.pathway_string)
                param_grid = {
